...
import { renderCompanyCertificatesDemo } from '../../demo/realScreenDemo.js';
...
if (certActions) certActions.classList.remove('hidden');
  ...
  try {
    ...
    const { data: profile, error: profileErr } = await companyService.table()
      .select('id, name, afm')
...
export async function loadCertificates() {
  ...
  try {
    const { data, error } = await certificateService.company()
      .select('*')
...
function bindCertificateActions() {
  document.querySelectorAll('.view-btn').forEach(btn => btn.addEventListener('click', async () => {
    try {
      await openCertificatePreview('companycertificates', btn.dataset.ref, btn.dataset.title || 'Προβολή πιστοποιητικού');
    } catch (err) {
      handleError(err);
    }
  }));
  document.querySelectorAll('.delete-btn').forEach(btn => btn.addEventListener('click', async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        await certificateService.company().delete().eq('id', btn.dataset.id);
        await loadCertificates();
  ...
  document.querySelectorAll('.edit-btn').forEach(btn => btn.addEventListener('click', async () => {
    const { data: certs } = await certificateService.company().select('*').eq('id', btn.dataset.id);
    const cert = certs[0];
...
<input id="custom-type" class="swal2-input hidden" placeholder="Καταχώρισε τύπο">
      ...
      showCancelButton: true,
      didOpen: () => {
        const select = Swal.getPopup().querySelector('#swal-type');
        const custom = Swal.getPopup().querySelector('#custom-type');
      ...
      preConfirm: () => {
        ...
        const select = document.getElementById('swal-type');
        const custom = document.getElementById('custom-type');
        const type = select.value === 'Άλλο' ? custom.value.trim() : select.value;
        const date = document.getElementById('swal-date').value;
        if (!title || !type || !date) { Swal.showValidationMessage('Συμπλήρωσε τίτλο, τύπο και ημερομηνία λήξης.'); return false; }
        return { id: cert.id, title, type, date };
      }
    });
    if (value) {
      await certificateService.company().update(value).eq('id', value.id);
      await loadCertificates();
    }
  }));
}
...
async function notifyCompaniesForExpiringSupplierCerts() {
  try {
    const { data: companyProfile, error: profileErr } = await companyService.table()
      .select('id')
      .eq('user_id', currentUser.id)
      .maybeSingle();
    ...
    const { data: suppliers, error: supErr } = await relationshipService.table()
      .select('supplier_id')
    ...
    for (const s of suppliers) {
      const { data: certs, error: certErr } = await certificateService.supplier()
      ...
      for (const cert of expiring) {
        const { data: existing } = await notificationService.company()
          .select('id')
        ...
        if (!existing) {
          const { error: insertErr } = await notificationService.company().insert({
          ...
          if (insertErr) {
            ...
            const { data: supplierData } = await supplierService.table()
              .select('name, afm')
              .eq('id', s.supplier_id)
              .maybeSingle();
...
async function showExpirationPopup() {
  const { data } = await certificateService.company().select('*').eq('company_user_id', currentUser.id).order('date', { ascending: false });
  const soon = data.filter(c => {
    const diff = daysUntil(c.date);
    return diff >= 0 && diff <= 30;
  });
...
const type = selectedType === 'Άλλο' && customTypeInput?.value.trim() ? customTypeInput.value.trim() : selectedType;
    ...
    if (res.isConfirmed) {
      try {
        ...
        const { error: upErr } = await storageService.upload('companycertificates', path, file);
        ...
        const { error: insertErr } = await certificateService.company()
          .insert([{
